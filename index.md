...
## Epistemic logic
  ...
  - **Vote on the team make-up**: Each agent casts a vote. Again, it is important whether an agent is a Resistance member or a spy.
    - **Resistance**: 
    - **Spy**: 
  - **Playing the cards**: Of course, a Resistance agent will always play a pass-card. For a spy, the card played depends on whether the spies are using epistemic logic to make their decision. If they are not using epistemic logic, the spies will always play a fail card. If the spies are using epistemic logic, they will deterine whether playing a fail card will result in the identity of both spies being revealed. If this is the case, they will choose to play a pass card to prevent the identities from being revealed. Otherwise, they will play a fail card.
  - **Reasoning based on the outcome**: Firstly, a public announcement is made to all the agents about the result of the mission. This is used to update the Kripke model to reflect the knowledge gained from the announcement. Secondly, the resistance agents can use the result of the mission and the voting round to reason on the potential identity of the spies. 
